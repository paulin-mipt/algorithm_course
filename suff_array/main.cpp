...
void buildSuffixArray(const std::string& str, std::vector<size_t>& suffixArray) {
  ...
  std::vector<size_t> classesCount(std::max(classesNumber, textLength)); //number of substrs in i'th class
                                                                    //initialization
...
//simple counting, O(N)
size_t longestCommonPref(size_t samplePos, size_t textPos, const std::string& text, const std::string& sample) {
  size_t answer = 0;
...
//compare function
int cmpPrefix(size_t samplePos, size_t textPos, const std::string& text, const std::string& sample) {
...
void binSearch(size_t left, size_t right, size_t mlr, size_t& leftBound, size_t& rightBound,
  const std::vector<size_t>& pos, const std::string& text, const std::string& sample) {
  if (left > right) {
    return;
  }
...
void init(std::string& text, std::string& sample, std::vector<size_t>& suffixArray) {
  std::cin >> text;
  std::cin >> sample;
